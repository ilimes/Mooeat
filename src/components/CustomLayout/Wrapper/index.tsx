...
const Wrapper = ({ children, ...rest }: { children: React.ReactNode }) => {
  ...
  useEffect(() => {
    ...
    if (!lastPageRef.current) lastPageRef.current = currentPageRef.current.children;

    exitAnimationDivRef.current?.appendChild(lastPageRef.current![0].cloneNode(true));
